...
from wind_stat_config import in_iso_path1, in_iso_path2, in_iso_path3_dir, in_window, in_assetexc, mode_exec 
from wind_stat_config import out_path, out_cf_path0
from wind_stat_config import sel_asset
from wind_stat_config import inspect_figure
...
if mode_exec==1:
    ...
    try:
        ...
        iso_data2 = pd.read_csv(in_iso_path2, dtype={'val_capacinstaladacomissconmw': float, 'val_geraenergiaconmwmed': float}, parse_dates=parse_dts, index_col=False)
        iso_data2 = iso_data2[['din_instante','nom_usina','dsc_estado','Fator de Carga Comiss','val_capacinstaladacomissconmw','val_geraenergiaconmwmed']]
        files_path=glob.glob(os.path.join(in_iso_path3_dir,'*.csv'))
        iso_data3_list = [pd.read_csv(file, dtype={'val_capacinstaladacomissconmw': float, 'val_geraenergiaconmwmed': float}, delimiter=';', parse_dates=parse_dts, index_col=False) for file in files_path]        
        iso_data3_yr = pd.concat(iso_data3_list, ignore_index=True)
        iso_data3_yr.rename(columns={'nom_estado': 'dsc_estado', 'nom_usina_conjunto': 'nom_usina', 'val_fatorcapacidade': 'Fator de Carga Comiss', 'val_capacidadeinstalada': 'val_capacinstaladacomissconmw', 'val_geracaoverificada': 'val_geraenergiaconmwmed'}, inplace=True)
        iso_data3_sel = iso_data3_yr[iso_data3_yr['nom_tipousina']=='Eólica']
        iso_data3 = iso_data3_sel[['din_instante','nom_usina','dsc_estado','Fator de Carga Comiss','val_capacinstaladacomissconmw','val_geraenergiaconmwmed']]
        iso_data4 = pd.concat([iso_data1, iso_data2, iso_data3], ignore_index=True)
    except FileNotFoundError as e:
        print(f"[!] Error loading files: {e}")
        exit(1)

    # create new columns with day, month, and year to group entries
    iso_data4['day'] = pd.DatetimeIndex(iso_data4['din_instante']).day
    iso_data4['month'] = pd.DatetimeIndex(iso_data4['din_instante']).month
    iso_data4['year'] = pd.DatetimeIndex(iso_data4['din_instante']).year

    # select subset to store
    df1 = iso_data4[['din_instante', 'nom_usina', 'dsc_estado', 'day', 'month', 'year', 'Fator de Carga Comiss', 'val_capacinstaladacomissconmw', 'val_geraenergiaconmwmed']].copy()
...
if mode_exec==4:
    ...
    def plot_comparison(project_name, index):
        ...
        # Automatically adjust subplot params so that the subplot(s) fits in to the figure area
        plt.tight_layout()
        figure_name = f"wind_inspect_{index}.png"   # Construct the filename
