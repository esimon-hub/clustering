...
Usage:
1. Configure parameters for pv_stat_config.py
...
from pv_stat_config import era5_src, map_path_folder
from pv_stat_config import yr_res1, yr_res2
from pv_stat_config import shapeon
...
# --- SHP ---
onshore_data = gpd.read_file(shapeon)
...
fig.suptitle('Solar radiation', fontsize=12, fontweight='bold')
# populate subplots
for i in range(n_rows):
    for j in range(n_cols):
        ...
        x,y = map(llons,llats)
        irrad=clipped.sel(season=iseason).values
        avrg = np.nanmean(irrad)
        std_dev = np.nanstd(irrad)
        # add text below each figure
        text_str = f'\u03BC: {avrg: .2f} kWh/m²/day\n\u03C3: ±{std_dev: .2f} kWh/m²/day'
        ax.text(0.5,-0.07, text_str, transform=ax.transAxes, ha='center', va='top', fontsize=10)
        temp_plot = map.contourf(x, y, irrad, levels=levels, cmap=cmap_sel, norm=Normalize(vmin=vmin, vmax=vmax), extend='both')
...
cb.set_label('Long-term daily average of solar radiation (kWh/m²/day)', fontsize=10)
cb.ax.tick_params(labelsize=10)
# save plot into a file
figure_name = f"pv_map_matrix.png"  # Construct the filename
save_path = os.path.join(map_path_folder, figure_name)  # Combine the directory with the filename
...
norm = Normalize(vmin=vmin, vmax=vmax)
mappable = plt.cm.ScalarMappable(norm=norm, cmap=cmap_sel)
fig.suptitle('Solar radiation', fontsize=10, fontweight='bold')
# populate subplots
for i in range(n_cols):
    ...
    temp_plot = map.contourf(x, y, irrad, levels=levels, vmin=vmin, vmax=vmax, cmap=cmap_sel, shading='interp', extend='both')
# create a single colorbar
cbar_ax = fig.add_axes([0.15, 0.17, 0.7, 0.03])
cb = ColorbarBase(cbar_ax, cmap=cmap_sel, norm=norm, orientation='horizontal', extend='both', ticks=ticks)
cb.set_label('Long-term daily average of solar radiation (kWh/m²/day)', fontsize=9)
cb.ax.tick_params(labelsize=9)  # Adjust tick label size for clarity
# save plot into a file
figure_name_png = f"pv_map_uni.png"  # Construct the filename
figure_name_pdf = f"pv_map_uni.pdf"
